...
public abstract class AsyncTask<Params, Progress, Result> {
     ...
     */
    private static final Executor SERIAL_EXECUTOR = (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) ? new SerialExecutor() :
    ...
    /**
     * Creates a new asynchronous task. This constructor must be invoked on the UI thread.
     * @param priority - a value from -20 (highest priority) to 19 (lowest priority)
     */
    public AsyncTask(final int priority) {
        mWorker = new WorkerRunnable<Params, Result>() {
            @Override
            public Result call() throws Exception {
                mTaskInvoked.set(true);
